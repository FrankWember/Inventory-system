...
import { fmt, fmtNum, fmtShort, fmtShortBare, dateLabel, dateLabelLong, getCategoryColor, today } from '../utils/helpers'
  ...
  if (loading) {
    return (
      <View style={[styles.wrapper, { backgroundColor: colors.surface }]}>
        <ScreenHeader title={t('stats.title')} subtitle={t('stats.lastDays', { period })} colors={colors} />
      ...
      >
        <View style={styles.periodSelector}>
          {[7, 30, 90].map(p => (
            ...
            >
              <Text style={[styles.periodButtonText, { color: period === p ? colors.white : colors.slate }]}>{t('stats.daysShort', { count: p })}</Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        <View style={styles.kpiRow}>
          <View style={styles.kpi}>
            <Text style={[styles.kpiLabel, { color: colors.slate }]}>{t('stats.revenue')}</Text>
            <Text style={[styles.kpiValue, { color: colors.slateDark }]} numberOfLines={1}>
              {fmtShort(totalRevenue)}
            </Text>
          </View>
          <View style={styles.kpi}>
            <Text style={[styles.kpiLabel, { color: colors.slate }]}>{t('stats.profit')}</Text>
            <Text
          ...
          <View style={styles.kpi}>
            <Text style={[styles.kpiLabel, { color: colors.slate }]}>{t('stats.soldLabel')}</Text>
            <Text style={[styles.kpiValue, { color: colors.slateDark }]}>{fmtNum(totalUnits)}</Text>
          </View>
        </View>
        ...
        <Card>
          ...
          <CardContent>
            {drinkPerformance.slice(0, 15).map((item, i) => (
              <View key={item.drink.id} style={[styles.performanceItem, { borderBottomColor: colors.border }]}>
                <Text style={[styles.performanceNumber, { color: colors.slate400 }]}>{i + 1}</Text>
                <View style={styles.performanceInfo}>
                  <Text style={[styles.performanceName, { color: colors.slateDark }]} numberOfLines={1}>{item.drink.name}</Text>
                  <Text style={[styles.performanceDetails, { color: colors.slate }]}>
                ...
                <View style={styles.performanceRight}>
                  ...
                  <Text style={[styles.performanceRevenue, { color: colors.slateDark }]}>{fmt(item.revenue)}</Text>
                </View>
              </View>
            ))}
            {drinkPerformance.length === 0 && (
              <Text style={[styles.empty, { color: colors.slate }]}>{t('stats.emptyStats')}</Text>
            )}
          </CardContent>
        </Card>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  performanceNumber: { width: 22, fontSize: 14, fontWeight: '700' },
  performanceInfo: { flex: 1 },
  performanceName: { fontSize: 14, fontWeight: '600' },
  performanceDetails: { fontSize: 12, marginTop: 2 },
  performanceRight: { alignItems: 'flex-end', gap: 4 },
  performanceRevenue: { fontSize: 12, fontWeight: '700' },
  empty: { fontSize: 13, textAlign: 'center', padding: 12 },
})
